...
class AlgorithmBermFromZ(Algorithm):
    ...
    def _execute(self) -> DStability:
        ...
        while True:
            for h in np.arange(
                h,
                h + 5.1 * self.step_size,
                self.step_size,
            ):
                # while sf < self.required_sf:
                ds = deepcopy(self.ds)
                ds.name = f"berm_{h:.2f}"
                ...
                p3 = (
                    self.ds.right,
                    p1[1]
                    + math.sin(math.radians(self.angle)) * (self.ds.right - p1[0]),
                )
            ...
            runner_results = runner.execute()
            runner_results = [
                result for result in runner_results if result.fos >= self.required_sf
            ]
            final_results = []
            if len(runner_results) > 0:
                for i in range(len(runner_results)):
                    final_results.append(
                        (
                            (float(runner_results[i].model.name.split("_")[-1])),
                            runner_results[i],
                        )
                    )
                # sort on berm
                final_results = sorted(final_results, key=lambda e: e[0])
                return final_results[0][1].model
